...
const _delete: DeleteFn = async (transaction) => {
  ...
  switch (response.body.result) {
    case 'deleted':
      return ok({ _id: response.body._id })
    case 'noop':
      return err(new NotModifiedError())
    case 'not_found':
      return err(new NotFoundError(`${transaction._id} Not Found`))
  }
...
const get: GetFn = async (arg) => {
  ...
  if (!response.body.found) return err(new NotFoundError(`${arg._id} not found`))
  if (response.body._source) return ok(response.body._source)

  throw new Error()
}

export async function createTransactionIndex(
  client: NewTypes,
): Promise<ApiResponse<estypes.CreateIndexResponse, unknown>> {
